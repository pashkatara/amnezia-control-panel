...
{
    private static array $knownDescriptions = [
        'amnezia-wg' => 'Модифицированный протокол WireGuard с обфускацией заголовков',
        'amnezia-wg-advanced' => 'AmneziaWG 3.1 с защитой от DPI и Junk-пакетами',
        'awg31' => 'AmneziaWG 3.1 с защитой от DPI и Junk-пакетами',
        'awg2' => 'AmneziaWG с настраиваемыми H-параметрами',
        'wireguard' => 'Стандартный быстрый и защищенный протокол WireGuard',
        'openvpn' => 'Классический защищенный протокол OpenVPN',
        'shadowsocks' => 'Прокси-протокол Shadowsocks для обхода блокировок',
        'cloak' => 'Обфускация Cloak с маскировкой под HTTPS-трафик',
        'cf-warp' => 'Интеграция Cloudflare WARP для обхода ограничений',
        'xray-reality' => 'XRay VLESS Reality для обхода DPI',
        'aivpn' => 'AI VPN протокол с динамической маршрутизацией',
        'mtproxy' => 'Специализированный MTProxy для Telegram',
    ];

    private static function sanitizeProtocol(array $protocol): array
    {
        if (isset($protocol['name'])) {
            $protocol['name'] = Translator::fixMojibake($protocol['name']);
        }
        $slug = $protocol['slug'] ?? '';
        if (isset(self::$knownDescriptions[$slug])) {
            $protocol['description'] = self::$knownDescriptions[$slug];
        } elseif (isset($protocol['description'])) {
            $protocol['description'] = Translator::fixMojibake($protocol['description']);
        }
        return $protocol;
    }
    ...
    public static function getAllProtocolsWithStats(): array
    {
        try {
            ...
            return array_map([self::class, 'sanitizeProtocol'], $rows);
    ...
    public static function getProtocolWithDetails(int $protocolId): array
    {
        try {
            ...
            $protocol = self::sanitizeProtocol($protocol);
