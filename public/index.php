...
Router::post('/servers/create', function () {
    ...
    if ($creationMode === 'backup') {
        ...
        if (isset($_FILES['backup_upload']) && $_FILES['backup_upload']['error'] === UPLOAD_ERR_OK) {
            ...
            // If backup contains multiple servers and user has not chosen yet, ask them to pick
            if (count($serversMeta) > 1 && $serverIndexRaw === '') {
                $formData['uploaded_servers'] = $serversMeta;
                View::render('servers/create.twig', [
                    'info_message' => 'В файле резервной копии найдено несколько серверов (' . count($serversMeta) . '). Пожалуйста, выберите нужную конфигурацию:',
                    'selected_mode' => 'backup',
                    'form_data' => $formData,
                    'protocols' => $protocols,
                    'default_protocol' => $defaultProtocol
        ...
        try {
            if ($token === '') {
                throw new Exception('Загрузите файл резервной копии перед импортом');
            }
            ...
            if ($serverIndex < 0) {
                if (!empty($serversMeta)) {
                    ...
                    throw new Exception('Не удалось прочитать серверы из файла резервной копии');
                }
            } else {
                if (!empty($serversMeta)) {
                    $formData['uploaded_servers'] = $serversMeta;
            ...
            // Also discover and sync all existing peers/clients directly from VPS
            try {
                VpnServer::syncClientsFromRemoteServer($serverId);
            } catch (Throwable $syncError) {
                error_log('Sync clients after backup import error: ' . $syncError->getMessage());
            }
...
// Rename client
Router::post('/clients/{id}/rename', function ($params) {
    requireAuth();
    $clientId = (int) $params['id'];
    header('Content-Type: application/json');
    ...
    if ($newName === '') {
        http_response_code(400);
        echo json_encode(['success' => false, 'error' => 'Имя клиента не может быть пустым']);
        return;
    }
...
// Discover and sync all clients from VPS container (clientsTable & config)
Router::post('/servers/{id}/sync-clients', function ($params) {
    requireAuth();
    $serverId = (int) $params['id'];
    ...
    try {
        ...
        $res = VpnServer::syncClientsFromRemoteServer($serverId);
